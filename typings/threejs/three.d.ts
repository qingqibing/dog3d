...
declare module THREE {
   ...
   export class BufferGeometry extends Eventable implements IEventable {
      ...
      attributes: { [key: string]: BufferAttribute | InterleavedBufferAttribute };
      morphAttributes: any;
   ...
   export interface MaterialParameters {
      ...
      blending?: Blending;//混合模式
      blendSrc?: BlendingDstFactor;//源颜色因子
      blendDst?: BlendingSrcFactor;//目标颜色因子
      blendEquation?: BlendingEquation;//混合方程式
      depthTest?: boolean;//是否开启深度测试
      depthWrite?: boolean;//深度是否可写
      polygonOffset?: boolean;//多边形位移,当两个面共面时,会出现十分难看的z - fighting 问题,要解决此问题可以使用, Polygon Offset,
      polygonOffsetFactor?: number;  //多边形位移因子
      polygonOffsetUnits?: number; //多边形位移单位
      alphaTest?: number;//透明度测试
      overdraw?: number;//反走样重绘
      visible?: boolean;//是否可见
      needsUpdate?: boolean;//当设置为true时,标记材质已经更新.
   }
   ...
   export interface MeshBasicMaterialParameters extends MaterialParameters {
      ...
      needsUpdate?: boolean;//当设置为true时,标记材质已经更新.
      color?: number;//颜色
      map?: Texture;//uv map ,漫反射贴图
      aoMap?: Texture;//环境光遮蔽贴图
      aoMapIntensity?: number;//环境光遮蔽强度
      specularMap?: Texture;//高光贴图
      alphaMap?: Texture;//透明贴图
      envMap?: Texture;//环境贴图
      combine?: Combine;//合并方式
      reflectivity?: number;//反射强度
      refractionRatio?: number;//反射率
      shading?: Shading;//材质
      wireframe?: boolean;//线框模式
      wireframeLinewidth?: number;//线框宽度
      vertexColors?: Colors;//顶点色
      skinning?: boolean;//是否参与蒙皮计算
      morphTargets?: boolean;//是否参与变形计算
      fog?: boolean;//是否启用雾效
   }

   //trhee.js基础材质参数，详见http://blog.csdn.net/omni360/article/details/42079869
   interface AllMeshBasicMaterialParameters {
      name?: string;//材质名称
      side?: Side;//双面显示
      opacity?: number;//透明度
      transparent?: boolean;//是否开启透明
   ...
   export class Box3 {
      ...
      setFromPoints(points: Vector4[]): Box3;
   ...
   export class Vector3 implements Vector {
       ...
       * Sets value of this vector.
       */
      set(x: any, y: number, z: number): Vector3;
   ...
   export class TextGeometry extends THREE.ExtrudeGeometry {
      ...
      constructor(text: string, parameters?: TextGeometryParameter);
      parameters: {
         size?: number;
         height?: number;
         curveSegments?: number;
   ...
   export class BoundingBoxHelper extends Mesh {
      constructor(object?: any, hex?: number);
      object: Object3D;
      box: Box3;
      update(): void;
   }
